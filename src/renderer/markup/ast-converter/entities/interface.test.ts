...
scope("Renderer", TypeKind.Interface, () => {

  {
    ...
    const renderedInterfaceForTableOfContents = convertInterfaceForTableOfContents(ctx, simpleInterface as InterfaceEntity);
    const renderedInterfaceForDocumentation = convertInterfaceForDocumentation(ctx, simpleInterface as InterfaceEntity);

    const interfaceName = Object.keys(renderedInterfaceForDocumentation)[0]! as AnchorIdentifier;
    const interfaceContent = convertedInterfaceForDocumentation[interfaceName]!;

    const documentation = convertAST(ctx, renderedInterfaceForDocumentation);
